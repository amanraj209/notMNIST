...
class notMNISTModel(BaseModel):
    def __init__(self, data_loader, config):
        super(notMNISTModel, self).__init__(config)
        self.data_loader = data_loader

        self.x = None
        self.y = None
        self.is_training = None
        self.is_validation = None
    ...
    def build_model(self):
        ...
        with tf.variable_scope('network'):
            conv1 = notMNISTModel.conv_bn_relu('conv1_block', self.x, 16, (3, 3), self.is_training)
            conv2 = notMNISTModel.conv_bn_relu('conv2_block', conv1, 32, (3, 3), self.is_training)

            with tf.variable_scope('max_pool1'):
                max_pool1 = tf.layers.max_pooling2d(conv2, pool_size=(2, 2), strides=(2, 2), name='max_pool')

            conv3 = notMNISTModel.conv_bn_relu('conv3_block', max_pool1, 32, (3, 3), self.is_training)
            conv4 = notMNISTModel.conv_bn_relu('conv4_block', conv3, 32, (3, 3), self.is_training)
            ...
            dense1 = notMNISTModel.dense_bn_relu_dropout('dense1', flattened, 512, 0.5, self.is_training)
            dense2 = notMNISTModel.dense_bn_relu_dropout('dense2', dense1, 256, 0.3, self.is_training)
